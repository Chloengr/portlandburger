...
router.post("/login", async function (req, res, next) {
  ...
  const encrypt_pass = auth.encrypt(req.body.password);
  // Checking
  const user = await db.User.findOne({
    where: { username: req.body.username, password: encrypt_pass.content },
  });
...
router.post("/register", async function (req, res, next) {
  ...
  const encypt_pass = auth.encrypt(user.password);
  await db.User.create({
    username: user.username,
    password: encypt_pass.content,
    role: user.role,
    id: user.id,
  }).then((result) => {
